...
class PointNet(nn.Module):
	...
	def features_grad(self, pc):
		...
		transform = self.feature_transform(feat)
		feat = torch.bmm(feat.permute(0, 2, 1), transform).permute(0, 2, 1)
		y = self.relu(self.bn3(self.conv3(feat)))
		
		y.retain_grad()

		z = self.relu(self.bn4(self.conv4(y)))
		z = self.relu(self.bn5(self.conv5(z)))
		z, idx = z.max(-1)

		# final MLP
		z = self.drop1(self.relu(self.bn6(self.fc1(z))))
		z = self.drop2(self.relu(self.bn7(self.fc2(z))))
		
		return z,y
	
	def layer2_features(self,pc):
		batch_size = pc.size(0)
		num_point = pc.size(2)
		assert pc.size(1)==3
		y = pc 
		y.retain_grad()
		transform = self.input_transform(y)
		z = torch.bmm(y.permute(0, 2, 1), transform).permute(0, 2, 1)
		z = self.relu(self.bn1(self.conv1(z)))
		z = self.relu(self.bn2(self.conv2(z)))

		transform = self.feature_transform(z)
		z = torch.bmm(z.permute(0, 2, 1), transform).permute(0, 2, 1)
